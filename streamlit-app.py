...
def load_stack_overflow_data():
    ...
    languages_df = pd.DataFrame(language_counter.items(), columns=['Language', 'Count']).sort_values(by='Count', ascending=False)
    return languages_df
...
def summarize_jetbrains_data(proglang_data):
    ...
    top_languages = language_counts.sort(by='Count', reverse=True).head(10)  # Now sorting and limiting to top 10 languages
...
def main():
    ...
    survey_selection = st.sidebar.radio(
        ...
        ('Statista Programming Survey', 'Stack Overflow Developer Survey', 'JetBrains Developer Ecosystem Survey 2022')  # Statista is listed first
    )
    ...
    if survey_selection == 'Statista Programming Survey':
        ...
        # Section content goes here (unchanged)
        ...
        fig_so = px.bar(languages_df.head(10), y='Language', x='Count', title='Top 10 Programming Languages Used According to Stack Overflow Survey', orientation='h')  # Limiting to top 10 languages
        fig_so.update_layout(yaxis={'categoryorder': 'total ascending'}, yaxis_title='Programming Language', xaxis_title='Count')  # Descending order on y-axis
